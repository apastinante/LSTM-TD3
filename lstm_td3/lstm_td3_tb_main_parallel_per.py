...
def simulate_beam_profile_PSB(params, phi, random_phase_shift, B_data=None, noise=None, imp=False, profile_slices=1000):
    ...
    with open(os.devnull, 'w') as devnull, \
        ...
        # Use pre-loaded B_data if provided, otherwise load it
        if B_data is None:
        ...
        try:
            ...
            matched_from_distribution_function(
                ...
                TotalInducedVoltage=total_induced_voltage if imp else None
            )
            profile.track()
...
class BeamEnvironmentFixed:
    ...
    def __init__(self, simulate_func, optimal_phases, max_ep_len, profile_slices, noise_level=0.0):
        ...
        # Pre-load magnetic field data once (always B_choice=1 based on the code)
        print("Loading magnetic field data...")
        try:
            self.B_data = load_magnetic_field(1)  # Always use choice 1
            print(f"✓ Magnetic field data loaded successfully")
        except Exception as e:
            print(f"✗ Error loading magnetic field data: {e}")
            self.B_data = None
    ...
    def reset(self, seed=None):
        ...
        # Generate profile
        result = self.simulate_profile(self.sim_params, self.current_phase_rel, self.random_phase_shift, B_data=self.B_data, profile_slices=self.profile_slices, noise=self.noise_level)
        try:
    ...
    def step(self, action):
        ...
        results = self.simulate_profile(self.sim_params, float(new_phase_rel), self.random_phase_shift, B_data=self.B_data, profile_slices=self.profile_slices, noise=self.noise_level)
        try:
...
def lstm_td3_parallel_per(resume_exp_dir=None,
                     env_name='', seed=0,
    ...
    for epoch in range(start_epoch, epochs):
        ...
        # Clear epoch storage
        mean_ep_ret = np.mean(epoch_storage['EpRet'])
